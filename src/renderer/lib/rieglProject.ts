// Is this dropped/opened path a RIEGL project directory?
//
// Both supported layouts are DIRECTORIES: `.riproject` from older instruments
// and `.PROJ` from newer ones (VZ-2000i and friends). react-dropzone expands a
// dropped folder into its contents — so by the time `onDrop` runs it has become
// ~100 .rxp/.jpg/.ppm files that the generic point-cloud importer rejects one by
// one ("Unsupported file format: .ppm"). The capture phase sees the un-expanded
...
// That anchoring matters more for `.PROJ` than it did for `.riproject`, since
// `.PROJ` nests its scans two levels deep.
export function isRieglProjectPath(path: string | undefined | null): boolean {
  if (!path) return false;
  return /\.(riproject|proj)\/?$/i.test(path);
}
