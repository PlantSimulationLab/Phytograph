...
// App+SceneProvider subtree, so no renderer-side scan state can leak into the
// re-import phase.
...
//   re-import that exported XML into a fresh scene → assert the re-imported scan's
...
test('round-trips a scanner model (RIEGL VZ-400i) through XML export and re-import', async () => {
  ...
  // ── Re-import the exported XML into a FRESH scene and assert the model came
  // back. File → New stands in for the old fresh-window relaunch: it remounts
  // the App subtree, so nothing renderer-side survives into the re-import.
  await resetToFreshScene(app, page);
  await stubOpenDialog(app, xmlPath);

  await page.getByTestId('tool-add-scan').click();
  const popup2 = page.getByTestId('scan-parameters-popup');
  await expect(popup2).toBeVisible();
  await page.getByTestId('scan-import-xml').click();
  await expect(popup2).not.toBeVisible({ timeout: 20_000 });
  await completeImportWizard(page);

  const rows2 = page.getByTestId('scans-panel').locator('[data-testid="scan-row"]');
  ...
  for (let i = 0; i < 4; i++) {
    ...
    await expect(popup2).toBeVisible();
    const val = await page.getByTestId('scan-model-select').inputValue();
