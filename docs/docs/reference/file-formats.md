...
| `.riproject` | ✅ | — | RIEGL **raw scanner project** — a *directory* of scan positions, not a file. Windows and macOS, and needs a user-supplied RiVLib (plus Docker on macOS only, where RiVLib has no native build): see **[Import a RIEGL project](../workflows/import-riegl-project.md)**. Carries reflectance, amplitude, deviation and per-pulse return numbering; scans arrive **unregistered**, though each position can be **levelled** using the instrument's own inclinometer (tilt only — not aligned to north or to each other); sky/miss points are recovered from the scanner's per-shot record, so LAD works (on Windows that recovery needs a C++ compiler; the scan imports without one, but with no sky shell). |
...
On **export**, the two structured formats keep misses in the form each one
...
- **E57** writes misses as cells flagged `cartesianInvalidState`. Here the box
  **does** matter: unticking it leaves the misses out entirely (a smaller,
  hits-only file), while ticking it keeps them, correctly flagged.
...
Each position also gets its own swatch color from the scan palette, exactly as
separately-imported files do — so the positions stay distinguishable in the
Scans panel and under the default **Per-scan color** mode.
