...
| POST | `/api/pointcloud/export` | `main.py` | Export a point cloud to LAS/LAZ — or, for octree-backed clouds (via a `source` descriptor), to any of LAS/LAZ/XYZ/TXT/CSV/PLY/OBJ (`obj` is still accepted here, but the Export window no longer offers it for point clouds — a `.obj` can only be re-imported as a mesh). The backend streams from the source file and applies any pending translation. Set **`dest_path`** (absolute) and the backend writes the file itself, returning metadata only with `data: null` — the app always does this, and it's the only form that works at scale: base64-in-JSON inflates the body ~1.8x, so a 25 M-point XYZ export lands near 1 GB and the renderer's `response.json()` throws `ERR_STRING_TOO_LONG` ("Unexpected end of JSON input"). Omitting `dest_path` keeps the legacy base64-in-`data` response for callers with no filesystem destination. A session exported to a LAS/LAZ file streams in 2 M-row blocks (`_export_session_to_las`) and never copies the survivor set |
...
| POST | `/api/cloud/session/{id}/delete_region` | `main.py` | Set the per-point deleted mask for points in a `region` (box/polygon/squares_union). Instant — array mask only, no rebuild. **Excludes `is_miss` points from the selection** (a crop only ever deletes hits). Returns counts, plus `backfilled_misses_stale: true` when the crop invalidated a separately-backfilled miss buffer (kept and still usable — LAD restores deletions outside the voxel grid and clears the flag itself, warning only when points were deleted from *inside* it, which a re-backfill cannot repair). Clears `octree_cache_id` (the cached octree no longer describes the points) but carries it into `rendered_octree_cache_id`, which `_evict_octree_cache` pins — the renderer keeps streaming from that directory and masks the deleted points until a rebuild lands, so it must not be evicted underneath a visible cloud |
...
| POST | `/api/cloud/session/{id}/reset_label_edits` | `main.py` | Undo labelling: reverse-apply the stored per-point deltas, newest first. Prefer **`undo_after_stroke_ids`** (the ids the renderer still holds, NEWEST FIRST) over `edit_count` for a global undo — the renderer's stroke list counts USER GESTURES while this history counts RECORDED CHANGES, and a stroke whose from-class gate matched nothing is never recorded, so the two legitimately diverge and a count silently rolls back the wrong edit. The session keeps up to the first id it recognises (an empty list keeps nothing). `stroke_id` is the join key between the two lists |
| POST | `/api/cloud/session/{id}/bake` | `main.py` | Permanently apply deletions — rebuild the octree from the surviving array points (`_session_to_las` → PotreeConverter), compact the arrays, clear the mask. Also reprojects + rebuilds the miss octree from the survivors, returned as `miss_octree_cache_id`. The one deliberately-slow step. No file read. The session is resolved before the response opens (a missing session is a clean 404); the rebuild then **streams PHP1 progress markers** ahead of the JSON tail and is **cancellable** via `/api/cancel/{run_id}` (SIGKILLs the PotreeConverter child). Every session mutation happens AFTER the build returns, and a cancel checkpoint sits between the two, so a cancel — including one landing as the converter finishes — leaves the pending deletions exactly as they were rather than half-applying them. Returns `deleted_history_len`: the undo depth that survived the bake, normally 0, non-zero only when a delete landed after the compaction. The renderer's `pendingDeletes` stack mirrors that history one-for-one (its entries index `reset_edits`), so it truncates to this rather than assuming the bake absorbed everything — which matters now that an applied crop's bake runs in the background while the user can still edit. `?compact=false` is that background display refresh: it rebuilds the octree but **keeps the deleted rows** under the mask (LAD restores the deleted hits outside its grid from them) and ends undo at the mask. Only the default compacting bake (**Permanently apply deletions**) removes rows, and it counts the removed hits so LAD warns. Both return `pending_deleted_count`, the renderer's `pendingDeletedCount` against the octree just built (also returned by `delete_region`, `reset_edits` and a non-rebuilding `filter`) |
...
| POST | `/api/cloud/session/{id}/extract` | `main.py` | Create a NEW child session from the filter-selected points, parent untouched. Always carries the parent's sky/miss points onto the child. Single-subset; the ground and per-tree splits both use the batch `extract_by_column` below |
| POST | `/api/cloud/session/{id}/extract_by_column` | `main.py` | Batch form of `extract`: fan a categorical column (`{slug}`, e.g. `tree_instance`) out into ONE child session per distinct value, parent untouched. `exclude_values` defaults to `[0]` (ground/miss/unassigned). `rebuild_parent` (default `false`) additionally rebuilds the PARENT's octree in the same concurrent pool as the children and returns it as `parent` — it pairs with `defer_octree` on the segmentation endpoints so the parent's rebuild, the single biggest cost of a classify-then-split run, overlaps the children's instead of running alone to completion first (measured 15.8 s → 11.7 s on a 1.2 M-point 2-way split). `include_misses` (default `false`) additionally duplicates the parent's sky/miss rows onto EVERY child, matching what single-shot `extract` always does — the ground/plant split sets it so each half keeps LAD's Beer's-law transmission denominator, while the per-tree split leaves it off so ~100 tree clouds don't each carry a full copy of the miss shell. Powers **split into one cloud per tree** and **split into ground + plant clouds**. All subsets are sliced under a single lock — one stable argsort of the column, then a per-child gather by absolute row index, so a K-way split costs O(N) in total rather than K full-survivor copies — and the per-child octree builds then run concurrently in a bounded pool (`_EXTRACT_BUILD_POOL`). The slug is validated before the response opens (a typo is a clean 400); the builds then **stream PHP1 progress markers** ahead of the JSON tail so the renderer can show a cancellable *Building n of N clouds…* pill. A cancel or failure drops the child sessions it had already registered, so a half-finished split doesn't strand a second copy of the cloud in RAM. The cancel is observed both at each build's start AND after each completion, and the run's cancel Event is forwarded into `_session_rebuild` so PotreeConverter is SIGKILLed — without the latter two, a split whose K fits the pool (the 2-way ground split always does) had every build past its only checkpoint before the user could click Cancel, and the children leaked |
...
| POST | `/api/cloud/session/{id}/segment_ground` | `main.py` | Run CSF on the array, append a `ground_class` column, rebuild from arrays. `defer_octree` skips the rebuild and returns `{octree_deferred: true}` with **no octree fields at all** (returning the pre-column octree with a `cache_id` would be the stale-octree bug `bake` guards against) — for a caller that immediately POSTs `extract_by_column` with `rebuild_parent`. Estimates the run's wall time and memory first (`_cost_advisory`): past the guideline it answers **409** with a structured `cost_warning`, and the caller re-sends with `acknowledge_cost: true`; a cloth too fine for the extent (>25 M cloth nodes) is refused with **400** and the coarsest resolution that fits. Large clouds run per buffered tile (`tiled.py`); the response's `tiled` field carries the plan |
...
| POST | `/api/cloud/session/{id}/segment_wood` | `main.py` | Wood/leaf segmentation on the in-RAM survivors → append `wood_class` → rebuild the octree. No file read. The compute runs in a **killable subprocess** so Cancel can SIGKILL it; the column write + rebuild happen in the parent afterwards, so a cancel mid-compute leaves the session pristine. The renderer's "split into wood + leaf clouds" then fans `wood_class` out through `extract_by_column` (`include_misses` + `rebuild_parent`), not two serial `extract` calls. Supports the same `defer_octree` as `segment_ground` |
...
| POST | `/api/cloud/session/{id}/backfill-misses` | `main.py` | Recover sky/miss points and persist them in a lightweight per-session buffer (`CloudSession.backfilled_misses`). Builds an ephemeral PyHelios cloud from the scan **as measured** — deleted hits are restored via `_session_to_lad_arrays(restore_mask=...)`, because the gapfill synthesises a miss into every raster cell with no return, so gap-filling over crop survivors alone would resurrect each cropped pulse as a transmitted beam (`restored_deleted_hits` reports how many were fed back) — runs `gapfillMisses()` (auto-selects the row/column or timestamp path; `row_index`/`column_index` are relabelled to the bare `row`/`column` keys the C++ dispatcher probes), and slices the synthesised misses via the bulk getters. Hit arrays are untouched. Rebuilds the projected-miss octree and returns its `miss_octree_cache_id`. Session-resolve + eligibility run up front (404 / 400-when-no-timestamp-or-grid); the heavy build/gapfill/extract **streams PHP1 progress markers** ahead of the JSON tail (`_do_backfill_misses` + `_bin_frame_streaming_response`) so the renderer shows a per-stage progress bar. Short-circuits (plain JSON) when the scan already has misses; a Helios reconstruction failure (too-sparse grid) returns an `error` field in the JSON tail rather than a 500 |
