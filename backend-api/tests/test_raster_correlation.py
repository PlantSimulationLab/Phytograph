...
_PEACH_TARGET = _PEACH_UNREG / "unregistered_ScanPos001.laz"
...
_UCD_DIR = (Path(__file__).resolve().parents[2] / "example-datasets"
            / "UCD_farm_registration")
_UCD_TARGET = _UCD_DIR / "UCD_unregistered_ScanPos001.laz"
_UCD_SOURCE = _UCD_DIR / "UCD_unregistered_ScanPos003.laz"
_UCD_TRUTH_1 = _TRUTH_DIR / "UCD_TRUTH_1.npy"
_UCD_TRUTH_3 = _TRUTH_DIR / "UCD_TRUTH_3.npy"


@pytest.mark.skipif(
    not (_UCD_TARGET.is_file() and _UCD_SOURCE.is_file()
         and _UCD_TRUTH_1.is_file() and _UCD_TRUTH_3.is_file()),
    reason="UCD_farm_registration fixtures not available (local-only example dataset)",
)
def test_shortlist_ranking_is_robust_to_its_iteration_budget():
    """The ranking must be judged on POSE, not on which candidate index wins.

    On this pair the two leading candidates finish within 0.001 of each other
    on inlier RMSE (0.3786 against 0.3778), so `_RANK_ITERATIONS` does change
    which index is ranked first: 60 and 30 agree, 20 and below pick a different
    one. That looks alarming and is not -- those near-tied candidates converge
    to EQUIVALENT poses, and the delivered pose sits at 0.07-0.13 m against
    RiSCAN at every budget from 60 down to 5.

    This test exists to keep that distinction straight, because reading the
    index alone once led to a "measured floor of 30" that the pose data does
    not support. It asserts the property that actually matters and asserts it
    across the range, so a future change to the budget is judged by where the
    scan lands rather than by an implementation detail.
    """
    target = _load_peach(_UCD_TARGET)
    source = _load_peach(_UCD_SOURCE)
    truth = np.linalg.inv(np.load(str(_UCD_TRUTH_1))) @ np.load(str(_UCD_TRUTH_3))
    want = source @ truth[:3, :3].T + truth[:3, 3]

    def pose_error(result):
        M = np.asarray(result["transformation"], dtype=np.float64)
        moved = source @ M[:3, :3].T + M[:3, 3]
        return float(np.median(np.linalg.norm(moved - want, axis=1)))
