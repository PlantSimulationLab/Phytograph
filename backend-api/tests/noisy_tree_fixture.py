...
if __name__ == "__main__":
    ...
    header = ("# synthetic noisy tree — see backend-api/tests/noisy_tree_fixture.py\n"
              f"# trunk {len(grp['trunk'])} (1 cm) · twigs {len(grp['twigs'])} (5 cm) · "
              f"flyers {len(grp['flyers'])} · clump {len(grp['clump'])} · "
              f"total {len(pts)}\n")
