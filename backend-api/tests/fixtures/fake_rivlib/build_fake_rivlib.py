...
#
# On macOS the NAME and the FORMAT deliberately disagree: the host asks for
# ".so" (its real runtime bind-mounts a Linux RiVLib into a container), while
# the compiler here emits Mach-O. That is fine only because these tests force
# the native runtime, where the reader dlopens the file into this process and
# therefore wants the host's own format — see _riegl_rivlib_unloadable, which
# checks Mach-O rather than ELF for exactly that combination.
