...
- **Build outputs** (`dist-main/`, `dist-preload/`, `dist-renderer/`, the electron-builder output dir, `resources/phytograph_backend/`) are all gitignored and produced by the scripts above — don't hand-edit them.
- **electron-builder output lives OUTSIDE the repo on local builds**: `~/builds/phytograph.noindex` (resolved by `scripts/build-output-dir.mjs`, injected via `scripts/run-electron-builder.mjs`). Reason: electron-builder unpacks a full `Phytograph.app` under `<output>/mac*/` next to the installers; when that sat in the Dropbox tree, Launch Services registered the stale copies and Phytograph showed up three times in the Apps launcher/Spotlight (plus ~2.8 GB of pointless Dropbox sync). The `.noindex` suffix keeps Spotlight out. **CI still uses the repo-relative `release/`** — `.github/workflows/release.yml` refers to `release/latest-mac.yml` and `find release -maxdepth 2 -name '*.app'`, and a `~/builds` path would be wrong on the Windows runner; the resolver returns `release` whenever `CI` is set. Override anywhere with `PHYTOGRAPH_BUILD_OUTPUT`. Only finished DMG/ZIP artifacts belong in Dropbox — never unpacked `.app` bundles.
...
6. **Share one app per spec file.** Multi-test spec files launch ONE app in `beforeAll` and reset with `resetToFreshScene` (`tests/e2e/helpers/resetApp.ts`, drives the real File → New) in `beforeEach` — see `bulk-actions.spec.ts` for the reference. A per-test `launchApp()` (~5-40s each) is only for tests about launch lifecycle itself (fresh-boot splash, octree cache recovery, app close semantics); comment why. The suite runs 2 Playwright workers — parallel spec files each get their own app, backend port, and octree cache dir, so never assume a fixed port or a solo instance.
